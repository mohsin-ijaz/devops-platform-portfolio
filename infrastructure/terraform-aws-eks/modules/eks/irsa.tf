...
resource "aws_iam_role" "aws_lb_controller" {
  count = var.enable_aws_lb_controller_irsa ? 1 : 0
...
# ArgoCD Application Controller IRSA (for cross-cluster management)

resource "aws_iam_role" "argocd_controller" {
  count = var.enable_argocd_irsa ? 1 : 0

  name = "${var.name}-argocd-controller-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "${local.oidc_provider_url}:sub" = "system:serviceaccount:argocd:argocd-*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy" "argocd_controller" {
  count = var.enable_argocd_irsa && length(var.argocd_target_role_arns) > 0 ? 1 : 0

  name = "argocd-cross-cluster"
  role = aws_iam_role.argocd_controller[0].id
    ...
    Statement = [
      {
        ...
        Resource = var.argocd_target_role_arns
      }
    ]
  })
}
